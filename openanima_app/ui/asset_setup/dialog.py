...
from ...assets.analyzer import AssetGuess
from ...assets.models import AssetType
from ...assets.validation import validate_asset_metadata
from ...rendering.metadata_renderers import CompositeUIRenderer
from ...rendering.metadata_renderers import sprite_strip_crop_values, sprite_strip_frame_size, sprite_strip_frames_from_pixmap
from .composite_canvas import CompositePreviewCanvas
from .helpers import (
    PREVIEW_TERMS,
    default_layer_for_image as _default_layer_for_image,
    default_name as _default_name,
    float_value as _float,
    image_files as _image_files,
    int_value as _int,
    matches as _matches,
    normalized_layer as _normalized_layer,
    readable_image as _readable_image,
)
from .undo import (
    end_coalesce as end_dialog_coalesce,
    redo as redo_dialog,
    restore_snapshot as restore_dialog_snapshot,
    snapshot as dialog_snapshot,
    push_undo_state as push_dialog_undo_state,
    undo as undo_dialog,
)
...
class AssetSetupDialog(QDialog):
    def __init__(self, source_path, guesses, existing_metadata=None, parent=None, primary_button_text=None):
        ...
        heading = QLabel("Asset Setup")
        heading.setObjectName("SectionTitle")
        source = QLabel(f"Source: {self.source_path}")
        source.setObjectName("SubtleLabel")
        ...
        top.addWidget(QLabel("Manual override"))
        top.addWidget(self.type_combo)
        layout.addLayout(top)

        body = QSplitter(Qt.Horizontal)
        self.guess_list = QListWidget()
        self.guess_list.setObjectName("DetectedTypeList")
        self.guess_list.setMinimumWidth(170)
        self.guess_list.setMaximumWidth(260)
        self.guess_list.currentRowChanged.connect(self.select_guess)
        for guess in self.guesses:
            item = QListWidgetItem(f"Detected type\n{guess.guessed_type} ({guess.confidence:.0%})")
        ...
        if ok_button is not None:
            ok_button.setText(primary_button_text or ("Save Metadata" if existing_metadata else "Import Asset"))
    ...
    def build_composite_page(self):
        ...
        layer_order_label = QLabel("Layer order: bottom to top. Drag items to reorder.")
        layer_order_label.setObjectName("SubtleLabel")
        layer_order_label.setWordWrap(True)
        left_layout.addWidget(layer_order_label)
        ...
        center = QGroupBox("Canvas")
        center_layout = QVBoxLayout(center)
        center_layout.setContentsMargins(10, 14, 10, 10)
    ...
    def update_details(self):
        ...
        self.details_label.setText(f"Detected type: {guess.guessed_type} ({guess.confidence:.0%})\n\nWhy OpenAnima chose it:\n{reasons}")
    ...
    def snapshot(self):
        return dialog_snapshot(self)

    def restore_snapshot(self, snapshot):
        restore_dialog_snapshot(self, snapshot)

    def push_undo_state(self, description="", coalesce_key=None):
        push_dialog_undo_state(self, description, coalesce_key)

    def end_coalesce(self):
        end_dialog_coalesce(self)
    ...
    def metadata(self):
        ...
        if asset_type in {AssetType.GIF, AssetType.APNG, AssetType.WEBM, AssetType.STATIC_IMAGE}:
            metadata = {"type": asset_type, "name": self.asset_name()}
            if self.source_path.is_file():
                metadata["image" if asset_type != AssetType.WEBM else "video"] = self.source_path.name
            return metadata
        guess = self.selected_guess()
        ...
        if self.source_path.is_file() and asset_type not in {AssetType.GIF, AssetType.APNG, AssetType.WEBM, AssetType.STATIC_IMAGE}:
