...
from .paths import is_supported_frame_file, natural_key, unique_folder_path
from ..rendering.webm import webm_likely_has_alpha
...
class AssetAnalyzer:
    ...
    def analyze_file(self, path: Path) -> list[AssetGuess]:
        ...
        if _is_apng_file(path):
            return [_apng_guess(path, "PNG file contains an APNG animation control chunk.")]
        ...
        if suffix in APNG_EXTENSIONS:
            return [_apng_guess(path, "File extension is .apng.")]

        if suffix in WEBM_EXTENSIONS:
            likely_alpha = webm_likely_has_alpha(path)
            metadata = {"type": WEBM_TYPE, "video": path.name}
            if likely_alpha:
                metadata["likely_alpha"] = True
            return [
                AssetGuess(
                    guessed_type=WEBM_TYPE,
                    confidence=0.85,
                    reasons=[
                        "File extension is .webm.",
                        "WebM playback uses Qt Multimedia; codec and alpha transparency support depend on the system backend.",
                    ]
                    + (["WebM metadata indicates alpha transparency."] if likely_alpha else []),
...
def _is_apng_file(path: Path) -> bool:
...
def _apng_guess(path: Path, reason: str) -> AssetGuess:
    return AssetGuess(
        guessed_type=APNG_TYPE,
        confidence=1.0,
        reasons=[
            reason,
            "APNG is handled before static image, sprite strip, and spritesheet heuristics.",
            "This Qt build may display only the first frame if animated PNG playback is unavailable.",
        ],
...
def _should_write_metadata(guess: AssetGuess) -> bool:
    return bool(guess.suggested_metadata) and guess.guessed_type not in {GIF_TYPE, APNG_TYPE, WEBM_TYPE, STATIC_IMAGE_TYPE}
